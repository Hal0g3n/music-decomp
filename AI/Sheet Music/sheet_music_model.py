...
# Gets an estimated tempo for the song, what more you want?
def getTempo(song, rate):
    return librosa.beat.tempo(song, rate).mean()


# Idea: 2 Transformers, 1 to determine note start, 1 to determine note presence
def createMIDI(song, rate, instrument, actlProb, onProb = None, onProb_threshold = 0.5, actlProb_threshold = 0.5):    
    ...
    onsets = (onProb > onProb_threshold).astype(np.int8) # Gets Onsets based on predicted probability
    frames = onsets | (actlProb > actlProb_threshold).astype(np.int8) # Ensure that any frame with an onset prediction is considered active.
    ...
    def EndPitch(pitch, endFrame):
        ...
        # Code untested (copied after all) so might break, do not uncomment pop statement below
        # intervals.pop(pitch)
